            ...
            {data.map((quiz) => (
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {authority == "NORMAL" && (
                    <button className="bg-[#242424] text-white px-3 py-1 rounded-xl text-sm hover:bg-blue-600"
                    >
                      Start
                    </button>
                  )}
                  {authority == "ADMIN" && (
                    <>
                      <button className="bg-[#242424] text-white px-3 py-1 rounded-xl text-sm hover:bg-green-600"
                      onClick={()=>{
                        navig(`/editquiz/${quiz.qId}`)
                      }}>
                        Edit
                      </button>
                      <button className="bg-[#242424] text-white px-3 py-1 rounded-xl text-sm hover:bg-yellow-600" onClick={()=>navig(`/questions/${quiz.qId}`)}>
                        Add question
                      </button>
                    </>
                  )}
